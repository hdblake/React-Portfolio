      ...
      <div className="mt-8 flex flex-wrap justify-center">
        {projects &&
          projects.map((project) => (
            <a href={project.link} key={project.id} className="p-4">
              <div className="flex relative shadow-xl rounded-2xl transition duration-300 ease-in hover:-translate-y-1 hover:scale-105">
                <img
                  src={project.image}
                  alt={project.title}
                  width={550}
                  height={343}
                  className="relative inset-0 object-cover object-center border border-2 border-accent1 rounded-2xl"
                />
                <div className="project-details absolute z-10 w-full h-full opacity-0 hover:opacity-95 bg-accent2 border border-2 border-main flex flex-col justify-center items-center gap-y-6 rounded-2xl">
